...
/**
 * BLACKJACK provider sync
 *
 * @author Alexander.B <alexbassmusic@gmail.com> - https://www.odesk.com/users/~01ae8f6e1a81c189cf
 */
require_once dirname(__FILE__).'/sync_products.php';

class Sync_products_blackjack extends Sync_products 
{
    public function __construct() 
    {
        parent::__construct();
        
        $this->_url_service = FCPATH .'upload/ofertas.csv';
        
        $this->_provider_name = 'BLACKJACK';
    ...
    protected function extract_products() 
    {
        ...
        foreach ($data_array as $row)
        {
            ...
            if( isset($ean) && preg_match('/^\d{6,13}/', $ean) )
            {
                ...
                if( $price <= floatval(1) || in_array((string)$ean, $this->_eans_to_exclude) || $product[4]<=1)
                {
                    $this->_products[$i]['stock']   = 0;
                }
                else 
                {
                    $this->_products[$i]['stock']   = (integer)$product[4];
                }
