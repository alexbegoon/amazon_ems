...
/**
 * COQUETEO2 provider sync
 *
 * @author Alexander.B <alexbassmusic@gmail.com> - https://www.odesk.com/users/~01ae8f6e1a81c189cf
 */
require_once dirname(__FILE__).'/sync_products.php';

class Sync_products_coqueteo2 extends Sync_products 
{
    public function __construct() 
    {
        parent::__construct();
        
        $this->_url_service = FCPATH .'upload/coqueteo2.csv';
        
        $this->_provider_name = 'COQUETEO2';
...
//        $this->_test_mode = TRUE;
    ...
    protected function extract_products() 
    {
        ...
        foreach ($data_array as $row)
        {
            ...
            if(isset($product[5]))
            {
                $price = (float)preg_replace('/,/', '.', $product[5]);
            }
            ...
            if( isset($ean) && preg_match('/^\d{6,13}/', $ean) )
            {
                ...
                $this->_products[$i]['inner_sku'] = $product[0];
                ...
                if( $price <= floatval(1) || in_array((string)$ean, $this->_eans_to_exclude) || (int)$product[2] <= 0 )
                {
                    $this->_products[$i]['stock']   = 0;
                }
                else 
                {
                    $this->_products[$i]['stock']   = (int)$product[2];
                }
            }
            else
            {
    ...
    protected function check_products_exceptions()
    {
        ...
        $xls_path = FCPATH . '/bloqueados_coqueteo2.xls';
